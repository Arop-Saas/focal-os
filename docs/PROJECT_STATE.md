...
_Last updated: 2026-07-24 (post-deliver-modal round). Update this file when the actual state changes; replace stale sections, don't append duplicates._
...
- **Services & Billing** card: order lines with pricing explanations (deletable, invoice stays in sync), add catalog OR custom one-off items with appointment mapping (none/existing/new), sleek inline invoice (no separate page), Collect payment (email invoice link / copy pay link / record payment), invoice per order created automatically at order creation.
- **Delivery**: the Deliver button (persists after delivery as Re-deliver) opens a pre-flight modal — editable email subject/message/CC, deliver silently (portal only), mark order as paid, marketing materials placeholder — then publishes the listing, emails, stamps and logs.
- **Clients on orders**: Client card shows outstanding balance, supports multiple clients per order (JobClient) and attaching a CustomerTeam; order identity (address/client/property) editable in place.
...
### Listings / delivery
- Listing system embedded in the order's Files & Delivery tab (`ListingManager`): photos/videos/tours/floorplans/other uploads, share link, settings (incl. per-listing "require payment to download"). Photo section: drag-to-reorder, multi-select (Cmd/Shift), keyboard shortcuts, fullscreen lightbox carousel; the cover is always the first photo. All previews via batch-signed URLs.
- Raw files tab: uploads + "Send to editor" (emails a team member the raw-files link with a note).
...
**See docs/CURRENT_TASK.md → "DEFERRED WORK" for the authoritative tracked list** (scheduling/permissions cluster, team notification preferences, marketing materials, invoice reconciliation, thumbnails).
