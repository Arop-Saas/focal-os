...
No task in flight. Latest round (2026-07-24, third): orders can have NO client — Job.clientId is nullable (remove the primary via the card X; the next client on the order is promoted, else the order goes clientless with an honest empty state; delivery falls back to silent-only, invoicing waits for a client). Invoice card gained Apply credit (draws the client's ClientCredit balance, records the payment) and Reminder (payment-reminder email, sent invoices only); the "updates with the order" note removed. Previous round (2026-07-24, second): invoice is a live mirror of the order (money re-synced on every billing change, "Not sent · updates with the order"); coupon apply/remove and removable tax on the Services & Billing card; dismissible Needs Attention (returns when reasons change); real order activity log via `logOrderActivity` (assigned, rescheduled, services, coupon/tax, payments, invoice sent, clients, team, delivery, uploads w/ collapse) — timeline no longer shows status-history noise; team-as-client card (team header, member list, primary contact selection, team-wide balance, teams in the Change picker).
