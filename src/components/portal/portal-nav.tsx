...
export function PortalNav({
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex h-screen w-56 flex-col bg-white border-r border-gray-100 shrink-0">
        {/* Logo */}
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white border-t border-gray-100 flex items-stretch safe-area-inset-bottom">
        {navItems.map((item) => {
          const active = pathname === item.href;
          return (
            <Link
        ...
        <a
          href={`/api/portal/logout?slug=${workspaceSlug}`}
          className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium text-gray-400"
        >
